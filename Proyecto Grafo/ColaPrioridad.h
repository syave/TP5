...
template<class T>
void ColaPrioridad<T>::acolar(T elemento, unsigned prioridad) {
    if (prioridad >= 0) {
...
template<class T>
void ColaPrioridad<T>::actualizar_valor(T vertice, unsigned valor) {
...
T ColaPrioridad<T>::desacolar() {
    if (this->esta_vacia()) {
        throw std::string("No se puede desacolar, la cola esta vacia.");
    }
...
T ColaPrioridad<T>::obtener_frente() {
    if (this->esta_vacia()) {
        throw std::string(
                "No se puede obtener el frente, la cola est� vac�a.");
    }
